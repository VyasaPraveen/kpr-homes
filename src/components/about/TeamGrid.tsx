  ...
  return (
    <section className="section-padding bg-charcoal-50">
      <div className="container-custom">
        <SectionHeading title="Meet Our Team" subtitle="The talented professionals behind every KPR Homes masterpiece" />

        <StaggerChildren staggerDelay={0.1} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {team.map((member) => (
            <motion.div
              key={member.id}
              variants={{
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300">
                <div className="relative h-64 overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-navy flex items-center justify-center">
                    <span className="text-5xl font-heading font-bold text-white/20">{member.name.charAt(0)}</span>
                  </div>
                ...
                <div className="p-5 text-center">
                  <h3 className="font-heading text-lg font-bold text-navy-900">{member.name}</h3>
